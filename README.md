...
5. Load the cluster with cluster density v2
5. Upgrade Cluster while running Kraken scenarios Node-memory-hog and Node-cpu-hog parallely
6. Perform must-gather if any failures during run
7. Destroy (based on parameter)

## Purpose
Run Loaded Upgrade workload on a given OpenShift cluster or it can create one for you based on ocp version, network type, installation type and cloud type. Run Kraken scenaario parallel to the Upgrade to achieve a more realistic scenario.
...
Scenario types are: node-cpu-hog, node-memory-hog
...
Shruti Yadav <@shyadav10 on Github>
